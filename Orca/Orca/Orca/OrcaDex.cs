...
namespace Orca
{
    ...
    public class OrcaDex: Dex
    {
        ...
        public override async Task<TransactionBuilder> GetSwapInstructions(
            PublicKey whirlpoolAddress,
            BigInteger amount,
            PublicKey inputTokenMintAddress,
            bool amountSpecifiedIsInput = true,
        ...
        )
        {
            SwapQuote swapQuote = await GetSwapQuoteFromWhirlpool(
                ...
                amountSpecifiedIsInput,
                commitment: commitment.GetValueOrDefault(DefaultCommitment));
            return await GetSwapWithQuoteInstructions(whirlpoolAddress, swapQuote, unwrapSol, commitment);
        }

        public override async Task<TransactionBuilder> GetSwapWithQuoteInstructions(
            PublicKey whirlpoolAddress,
            SwapQuote swapQuote,
            bool unwrapSol = true,
            Commitment? commitment = null
        ...
        )
        {
            ...
            for (var rewardIndex = 0; rewardIndex < position.RewardInfos.Length; rewardIndex++)
            {
                ...
                if(whirlpool.RewardInfos[rewardIndex].Mint.Equals(AddressUtils.DefaultPublicKey))continue;
        ...
        {
            if (mintAddress.Equals(AddressUtils.DefaultPublicKey)) return null;
