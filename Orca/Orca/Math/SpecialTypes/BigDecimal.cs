...
namespace Orca.Math.SpecialTypes
{
    internal static class SortingOrder
    {
    ...
    public readonly record struct BigDecimal : IComparable<BigDecimal>, IComparable<Int32>, IComparable<Int32?>, IComparable<Decimal>, IComparable<Double>, IComparable<Single>
    {
        ...
        public BigDecimal(Double value)
        {
            ...
            while (System.Math.Abs(value * scaleFactor - Double.Parse(mantissa.ToString())) > 0)
            {
                exponent--;
                scaleFactor *= 10;
        ...
        public BigDecimal(Single value)
        {
            ...
            while (System.Math.Abs(value * scaleFactor - Single.Parse(mantissa.ToString())) > 0)
            {
                exponent--;
                scaleFactor *= 10;
                mantissa = new BigInteger(value * scaleFactor);
        ...
        /// <summary>
        /// Returns the whole number integer part of the BigDecimal, dropping anything right of the decimal point. Essentially behaves like System.Math.Truncate(). For
        /// example, GetWholePart() would return 3 for System.Math.PI.
        /// </summary>
        ...
        //TODO Needs tests on E notation, negative numbers, and extra small or large numbers.
        ...
        */
        ...
        {
            var t = (BigDecimal)value.Mantissa * System.Math.Pow(10, value.Exponent);
            if (t > MaxBigDecimalForDouble.Value)
            {
                throw new Exception($"{nameof(BigDecimal)} is too large to convert to a {nameof(Double)}.");
            }
        ...
        public static explicit operator Single(BigDecimal value)
        {
            ...
            {
                throw new Exception($"{nameof(BigDecimal)} is too large to convert to a {nameof(Single)}.");
            }
        ...
        public static explicit operator Decimal(BigDecimal value)
        {
            ...
            {
                throw new Exception($"{nameof(BigDecimal)} is too large to convert to a {nameof(Decimal)}.");
            }
        ...
        public static explicit operator Int32(BigDecimal value)
        {
            ...
            {
                throw new Exception($"{nameof(BigDecimal)} is too large to convert to a {nameof(Int32)}.");
            }
        ...
        public static explicit operator UInt32(BigDecimal value)
        {
            ...
            {
                throw new Exception($"{nameof(BigDecimal)} is too large to convert to a {nameof(UInt32)}.");
            }
        ...
        {
            BigInteger wholeNumber = (BigInteger)System.Math.Truncate(exponent);
            Double fractionalPart = exponent % 1;
            int root = (int)System.Math.Truncate(1d / fractionalPart);
        ...
        public static BigDecimal Round(BigDecimal value, Int32 precision)
        {
            ...
            var sign = System.Math.Sign(exponent);
        ...
        private static String ToString(BigInteger mantissa, Int32 exponent, IFormatProvider provider)
        {
            ...
            var negativeExponent = System.Math.Sign(exponent) == -1;

            var result = BigInteger.Abs(mantissa).ToString();
            var absExp = System.Math.Abs(exponent);

            if (negativeExponent)
            {
                if (absExp > result.Length)
                {
                    var zerosToAdd = System.Math.Abs(absExp - result.Length);
                ...
                {
                    var indexOfRadixPoint = System.Math.Abs(absExp - result.Length);
